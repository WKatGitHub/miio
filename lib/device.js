...
module.exports = Thing.type(Parent => class extends Parent.with(Polling) {
	...
	constructor(handle) {
		...
		// Set up polling to destroy device if unreachable for 7.5 minutes
		this.updateMaxPollFailures(15);
	...
	defineProperty(name, def) {
		...
		if(def.name || name.did) { // miot
			this._reversePropertyDefinitions[def.name ? def.name : name.did] = name;
		}
		this._propertyDefinitions[name.did ? name.did : name] = def; // miot
	}
	...
	loadProperties(props) {
		...
		if(props[0].did){ // miot
			// Call get_properties to map everything
			return this.call('get_properties', props)
				.then(result => {
					const obj = {};
					for(let i=0; i<result.length; i++) {
						this._pushProperty(obj, result[i].did, result[i].value);
					} //if(this.handle.api.id === 334277515){ console.log(this.handle.api.id +' ---> data: '+ JSON.stringify(result)); }
					return obj; 
				});
		}
		...
		return this.call('get_prop', props)
			.then(result => {
				...
				}// if(this.handle.api.id === 54379364){ console.log(this.handle.api.id +' ---> data: '+ JSON.stringify(obj)); }
				return obj; 
			}); 
	}
	...
	/** miot
	 * Check that the current result is object with key code = 0.
	 */
	static checkCode0(result) {
		if(! result[0] || (typeof result[0].code === 'number' && result[0].code !== 0)) {
			throw new Error('Could not perform operation');
		}
